...
globalThis.Phaser = {
  Input: {
    Keyboard: {
      JustDown(key: { justDown?: boolean } | undefined) {
...
import { getAdamInput, getPlayerBumpOutcome, getPlayerRenderScale } from './players.ts';
...
test('player render scale includes temporary growth multiplier', () => {
  const player = {
    baseScale: 1.8,
    growthScale: 1.35,
  };

  assert.equal(getPlayerRenderScale(player), 2.43);
});
